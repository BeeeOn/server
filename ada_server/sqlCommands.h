...
const string UpdateModule = "update module set measured_value = :MEASURED_VALUE where (device_euid = :DEVICE_EUID AND module_id = :MODULE_ID);";  //"update devices set value=" + val  +" where (fk_facilities_mac='" + message->DeviceIDstr + "' AND type =" + std::to_string(message->values[i].intType) +");" ;

/** Q to insert history row uses ( ':deviceID', :timeStamp , :type, :value)*/
const string InsertLog = "insert into log (device_euid,measured_at,module_id,measured_value) values (:DEVICE_EUID,:MEASURED_AT,:MODULE_ID,:MEASURED_VALUE);"; //"insert into logs (fk_facilities_mac,timestamp,fk_devices_type,value) values ( '"+ message->DeviceIDstr + "', " + std::to_string(message->timestamp) + " , " + std::to_string(message->values[i].intType) + ", " + val + " );" ;
