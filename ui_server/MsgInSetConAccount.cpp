...
MsgInChangeConAccount::MsgInChangeConAccount(char* msg, pugi::xml_document* doc): IMsgInLoginAndAdapterAccessRequired(msg, doc) {
}
...
string MsgInChangeConAccount::createResponseMsgOut()
{
    ...
    for (; userNode; userNode = userNode.next_sibling(P_USER))
    {
        ...
        }else if(DBConnector::getInstance().changeConAccount(_adapterId, newUserMail, newRole) != 1){
            errText += "<user email=\""+newUserMail+"\" role=\""+newRole+"\"/>";
