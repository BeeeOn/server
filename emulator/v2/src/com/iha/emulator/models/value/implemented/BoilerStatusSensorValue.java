 ...
 * representing Boiler Status sensor. Uses Integer as value data type. Provides methods for storing, generating and saving value.
 * For generating new values uses both Normal and Linear distribution. All possible value provided by
 * {@link com.iha.emulator.models.value.implemented.BoilerStatusSensorValue.BoilerStatus} enum.
 *
 * @author <a href="mailto:xsutov00@stud.fit.vutbr.cz">Filip Sutovsky</a>
 */
public class BoilerStatusSensorValue extends AbstractValue<Integer> implements HasNormalDistribution,HasLinearDistribution,VPTRegulatorValue {

    /**
     * Enum implementing {@link com.iha.emulator.models.value.Status} interface. Provides all possible values with
     * their description.
     */
    public enum BoilerStatus implements Status {
        UNKNOWN(0,"Unknown"),
        HEATING(1,"Heating"),
        HEATING_UP_TUV(2,"Heating TUV"),
        ERROR(3,"Error"),
        SHUT_DOWN(4,"Shut down");
        /** value code */
        private int code;
        /** value description */
         ...
         */
        BoilerStatus(int code,String description){
            this.code = code;
    ...
    /**
     * Creates and initializes new instance of Boiler Status sensor from given information as parameter.
     ...
     */
    public BoilerStatusSensorValue(String name, String type, int offset, String unit, boolean generateValue, boolean storeHistory, Random generator, Long generatorSeed) {
        ...
        switch (getGeneratorType()){
            case NORMAL_DISTRIBUTION:
                if(devProperty() == null || avgProperty() == null || maxProperty() == null || minProperty() == null){
                    throw new IllegalArgumentException("Boiler status generator doesn't have variables needed to generate new value (Dev,Avg,Max,Min)");
                }
            ...
            case LINEAR_DISTRIBUTION:
                if(stepProperty() == null || maxProperty() == null || minProperty() == null){
                    throw new IllegalArgumentException("Boiler status generator doesn't have variables needed to generate new value (Step,Max,Min)");
                }
    ...
    public String toStringWithUnit() {
        for(BoilerStatus tmp : BoilerStatus.values()){
